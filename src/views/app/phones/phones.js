...
const DataListPages = ({
  ...
  const onSubmit = (event, errors, values) => {
    if (errors.length === 0 && state.images.length !== 0) {
      if (!isUpdate) {
        ...
        updatePhoneAction(state, state._id, (callBack) => {
          if (callBack.status === 200) {
            setModalOpen(!modalOpen);
            fetchNew();
            setIsUpdate(!isUpdate);
            createNotification('success', 'Berhasil update hp');
          }
        });
      }
    } else {
  ...
  const onChange = (e) => {
    ...
    if (name === 'front' || name === 'rear') {
      setState((prevState) => ({
        ...prevState,
        camera: { ...prevState.camera, [name]: value },
      }));
    } else if (name === 'fingerPrint' || name === 'nfc') {
  ...
  const onUploadImg = (file) => {
    ...
    setImages((newState) => [
      ...newState,
      { imageUrl: img.fileUrl, id: file.upload.uuid },
    ]);
  };
  ...
  const onDelete = () => {
    deletePhoneAction(selectedItems, (callBack) => {
      if (callBack.status === 200) {
        fetchNew();
      }
    });
  ...
  const toDetailModel = (id) => {
    history.push(`${adminRoot}/phones/details?id=${id}`);
  };
      ...
      <div className="disable-text-selection">
        ...
        <AddNewModal
          ...
          removeImage={(file) => {
            ...
            setImages((image) => image.filter((item) => item.id !== uuid));
          }}
        />
        <ListPageListing
          items={items}
...
export default connect(null, {
  submitModelAction: sumbitPhone,
  deletePhoneAction: deletePhone,
  publishModelAction: publishModel,
  updatePhoneAction: updatePhone,
})(DataListPages);
